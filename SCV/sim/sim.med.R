...
reg.funs[["Forward stepwise"]] = function(x,y) fs(x,y,intercept=FALSE)#, max=50)
...
for (beta.type in type.vec) {
  for (rho in rho.vec) {
    ...
    for (snr in snr.vec) {
      file = paste0("rds/med/", name, ".snr", round(snr,2), ".rds")
      cat("..... NEW SIMULATION .....\n")
      cat("--------------------------\n")
...
library(bestsubset)
n = 100; p = 10
file.list = system(paste0("ls rds/med/sim.n",n,".p",p,".*.rds"),intern=TRUE)
