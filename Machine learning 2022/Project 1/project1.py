...
df2[['MinTemp','MaxTemp', 'WindSpeed9am', 'Humidity9am', 'Pressure9am', 'Temp9am']].describe()
...
# Standardiser
Xtrans_stand = Xtrans - np.ones((N,1))*Xtrans.mean(0)
...
axs[0].hist(Xtrans_stand[:,3], bins=n_bins)
axs[1].hist(Xtrans_stand[:,4], bins=n_bins)
axs[0].set_title('WindGus')
...
# We can set the number of bins with the *bins* keyword argument.
axs[0,0].hist(Xtrans_stand[:,0], bins=n_bins)
axs[0,1].hist(Xtrans_stand[:,1], bins=n_bins)
axs[0,2].hist(Xtrans_stand[:,2], bins=n_bins)
axs[0,3].hist(Xtrans_stand[:,3], bins=n_bins)
axs[1,0].hist(Xtrans_stand[:,4], bins=n_bins)
axs[1,1].hist(Xtrans_stand[:,5], bins=n_bins)
axs[1,2].hist(Xtrans_stand[:,6], bins=n_bins)
axs[1,3].hist(Xtrans_stand[:,7], bins=n_bins)
...
df4 = df4.drop(df.columns[[8]], axis=1)
sns.pairplot(df4)
...
yticks, show,legend,imshow, cm, xlim, ylim)
