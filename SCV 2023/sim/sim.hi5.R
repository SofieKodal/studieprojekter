...
for (beta.type in type.vec) {
  for (rho in rho.vec) {
    ...
    for (snr in snr.vec) {
      file = paste0("rds/hi5/", name, ".snr", round(snr,2), ".rds")
      cat("..... NEW SIMULATION .....\n")
      cat("--------------------------\n")
...
save(list=ls(), file=paste0("rds/hi5/",stem,".rda"))
...
library(bestsubset)
n = 50; p = 1000
file.list = system(paste0("ls rds/hi5/sim.n",n,".p",p,".*.rds"),intern=TRUE)
...
plot.from.file(file.list, what="risk", rel.to=NULL, tuning="val",
               ...
               main=paste0("n=",n,", p=",p,", s=",s), ylim=c(0,3.5),
